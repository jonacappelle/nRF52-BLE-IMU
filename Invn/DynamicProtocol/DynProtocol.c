		...
		struct DynProtocolEdata * edata)
{
	...
	if(self->decode_state_machine.cmd_id == DYN_PROTOCOL_EID_PROTOCOLVERSION) {
		...
		{
			edata->d.command.cfg.base.type = buf[1];
			edata->d.command.cfg.size = buf[2];
		...
		struct DynProtocolEdata * edata)
{
	...
	case DYN_PROTOCOL_EID_GET_SENSOR_CFG:
	{
		...
		edata->d.response.sensorcfg.cfg.size = buf[2];
		...
		uint8_t * outBuffer, uint16_t maxBufferSize, uint16_t *outBufferSize)
{
	...
	*outBufferSize = 0;
	...
	case DYN_PROTOCOL_EID_SET_SENSOR_CFG:
		...
		outBuffer[idx++] = (uint8_t)edata->d.command.cfg.size;
		switch (edata->d.command.cfg.base.type) {
		case VSENSOR_CONFIG_TYPE_OFFSET:
			precision = DynProtocol_getPrecision(self, edata->sensor_id);
		...
		default:
			...
			if((maxBufferSize - idx) < edata->d.command.cfg.size)
				goto error_size;
			memcpy(&outBuffer[idx], edata->d.command.cfg.buffer, edata->d.command.cfg.size);
			break;
		}
		idx = 67 + 2;
		break;
		...
		uint8_t * outBuffer, uint16_t maxBufferSize, uint16_t *outBufferSize)
{
	...
	*outBufferSize = 0;
	...
	case DYN_PROTOCOL_EID_GET_SENSOR_CFG:
	{
		...
		outBuffer[idx++] = (uint8_t)edata->d.response.sensorcfg.cfg.size;
		switch (edata->d.response.sensorcfg.cfg.base.type) {
		case VSENSOR_CONFIG_TYPE_OFFSET:
			outBuffer[idx++] = (uint8_t)edata->sensor_id;
			precision = DynProtocol_getPrecision(self, edata->sensor_id);
		...
		default:
			// we don't know what is the value of cfg.size so let's make sure
			// there is enough room in outBuffer.
			if((maxBufferSize - idx) < edata->d.response.sensorcfg.cfg.size)
				goto error_size;
			memcpy(&outBuffer[idx], edata->d.response.sensorcfg.cfg.buffer, edata->d.response.sensorcfg.cfg.size);
			break;
		}
		idx = 68 + 2;
		break;
...
const char * DynProtocol_sensorTypeToStr(int type)
{
	...
	case DYN_PRO_SENSOR_TYPE_PREDICTIVE_QUATERNION:  return "PRED_QUAT";
	default:                                         return "";
	}
}
